...
export default function Home() {
	...
	const fetchSets = async (currentPage: number) => {
		...
		try {
			const res = await axios.get(
				`${API_URL}/StudySets?page=${currentPage}&pageSize=9`,
			);
	...
	const itemVariants = {
		...
		show: {
			opacity: 1,
			y: 0,
			transition: { type: "spring" as any, stiffness: 260, damping: 22 },
		},
	};
			...
			<div className="mb-10 flex justify-between items-end">
				<div>
					...
					<h1 className="text-4xl font-black text-slate-800">
						Discover{" "}
						<span className="text-blue-600">Study Sets</span>
					</h1>
					<p className="text-slate-500 mt-2 text-base">
						Browse flashcard sets created by the community
					</p>
				</div>
			</div>
							...
							<motion.div key={set.id} variants={itemVariants}>
								...
								>
									<div className="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm hover:shadow-lg hover:shadow-blue-100 hover:-translate-y-1 hover:border-blue-200 transition-all duration-300 group cursor-pointer h-full flex flex-col relative overflow-hidden">
										...
										<h2 className="text-lg font-bold text-slate-700 group-hover:text-blue-600 transition-colors mb-1 line-clamp-1">
											{set.title}
										</h2>
										<p className="text-slate-400 mt-1 line-clamp-2 flex-grow text-sm leading-relaxed">
											{set.description}
										</p>
